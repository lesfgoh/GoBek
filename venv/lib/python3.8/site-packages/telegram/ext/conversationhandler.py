...
class ConversationHandler(Handler[Update, CCT]):
  ...
  def conversation_timeout(
    self, ) -> Optional[Union[float, datetime.timedelta]]:
  ...
  def conversations(self, value: ConversationDict) -> None:
    ...
    for handlers in self.states.values():
      for handler in handlers:
        if isinstance(
            handler,
            ConversationHandler) and self.persistence and handler.name:
  ...
  def _schedule_job(
    ...
    if new_state != self.END:
      try:
        ...
        self.timeout_jobs[
          conversation_key] = j_queue.run_once(  # type: ignore[union-attr]
  ...
  def check_update(self, update: object) -> CheckUpdateType:  # pylint: disable=R0911
    ...
    self.logger.debug('selecting conversation %s with state %s', str(key),
                      str(state))
      ...
      if self.conversation_timeout:
        if dispatcher.job_queue is not None:
          ...
          if isinstance(new_state, Promise):
            ...
            self._schedule_job(new_state, dispatcher, update, context,
                               conversation_key)
        else:
          self.logger.warning(
  ...
  def _trigger_timeout(self,
    ...
    if isinstance(context, CallbackContext):
      job = context.job
      ctxt = cast(_ConversationTimeoutContext,
                  job.context)  # type: ignore[union-attr]
    else:
    ...
    for handler in handlers:
      ...
      if check is not None and check is not False:
        try:
          handler.handle_update(ctxt.update, ctxt.dispatcher, check,
                                callback_context)
